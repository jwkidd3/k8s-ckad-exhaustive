...
4. Identify the root cause for the connection issue and fix it. Two useful checks: `kubectl get endpoints web-app -n y72` (a Service with empty endpoints almost always means the selector doesn't match Pod labels), and `kubectl describe service web-app -n y72` (compare its `TargetPort` against the container port on the Deployment). Verify the fix by repeating the previous step. The `wget` call should return a successful response.
---
